...
export default function Schedule() {
  ...
  const [dataStation1, setDataStation1] = useState([])
  ...
  function setRealDate(date, hours) {
    if (parseInt(hours.split(':')[0]) > 23) {
      return (new Date(date) + 8640000)
    } else {
      return date
    }
  }
  ...
  async function getDataStation() {
    await axios.get('/station'
      ...
      setDataStation1(res.data.data1)
  ...
  async function getDataSchedule(date) {
    ...
    arrData.push(schedule)
    await axios.post('/schedule', {
      arrData: arrData
    }).then(function (res) {
      if (res.data.status === 'success') {
        setArrDataScheduleDetail(res.data.schedule[0])
            ...
            <div className="class-train-booking">
              ...
              <div className="d-fix">
                ...
                <div className="w40percent-left">
                  <div>Ga đế: {dataSchedule.stationTo.name}</div>
                  ...
                  {(dataStationForm && dataStationTo && dataStationForm.idShow < dataStationTo.idShow) ?
                    dataStation.map((item, index) => {
                      if (item._id === dataSchedule.stationFrom.id || item._id === dataSchedule.stationTo.id) {
                        return (
                          <tr key={index}>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{index + 1}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: '' }}>{item.name}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{item.distance}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{dataScheduleDetail && setRealTime(dataScheduleDetail.time, item.minutes)}</td>
                          </tr>
                    ...
                    dataStation1.map((item, index) => {
                      if (item._id === dataSchedule.stationFrom.id || item._id === dataSchedule.stationTo.id) {
                        return (
                          <tr key={index}>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{index + 1}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: '' }}>{item.name}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{1726 - item.distance}</td>
                            <td className="ng-binding train-in-header" style={{ textAlign: 'center' }}>{dataScheduleDetail && setRealTime(dataScheduleDetail.time, item.minutesRev)}</td>
                          </tr>
                  ...
                  {dataScheduleDetail?.train.carriage.map((item, index) => {
                    return (
                      ...
                      <tr tr key={index} >
                        ...
                        <td className="et-table-cell tabl-cell" style={{ textAlign: 'center' }}>{dataStationForm && dataStationTo && (format_curency((Math.abs(dataStationTo.distance - dataStationForm.distance) * item.unitPrice).toString()))}</td>
                      </tr>
                    )
                  })}
                </tbody>
